...
dwm_alsa () {
	...
	VOLSTAT=$(amixer get Master | grep "Mono:" | awk '{print $6}' | tr -d '[]')
        printf "%s" "$SEP1"
...
dwm_network() {
	CONNAME=$(nmcli -a | grep 'Wired connection' | awk 'NR==1{print $1}')
	 ...
	 printf "󰀂 %s %s|%s" "$CONNAME" "$PRIVATE" "$PUBLIC"
	 else
...
xsetroot -name "$(dwm_network) 󰍛 $(print_mem)M ⬇️ $vel_recv ⬆️ $vel_trans $(dwm_alsa) $(print_bat)$(print_date) "
